...
for filename in glob("/working/lab_nicw/olgaK/Pollock_endometrial_PDX_manuscript/data/processed/MAFs/*snp.filt_som.maf"):
	...
	out_vep = filename.replace('MAFs','MAFs/BROCA').replace('snp.filt_som.maf','filt_som.BROCA.input_vep.tsv')
	if not count % 10:
		print(f"Processing {count}th file")
	with open(out_vep,"wt") as fout:
		geneset = parse_maf(filename,fout,geneset,header=True)
		geneset = parse_maf(filename_indel,fout,geneset, header=False)
	count+=1
print(count)
print(geneset)
