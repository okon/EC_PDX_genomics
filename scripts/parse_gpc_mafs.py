...
for filename in glob("/working/lab_nicw/olgaK/Pollock_endometrial_PDX_manuscript/data/raw/MAFs/*gpc.maf.gz"):
	out_maf = filename.replace('raw/MAFs','processed/MAFs/BROCA').replace('.maf.gz','.BROCA.maf.gz')
	out_vcf = filename.replace('raw/MAFs','processed/MAFs/BROCA').replace('.maf.gz','.BROCA.vcf')
