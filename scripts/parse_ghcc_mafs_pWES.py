...
count=1
geneset=set()
for filename in glob("/working/lab_nicw/olgaK/Pollock_endometrial_PDX_manuscript/data/raw/MAFs/pWES/*ghcc.maf.gz"):
	out_maf = filename.replace('raw/MAFs/pWES','processed/MAFs/pWES_BROCA_germ').replace('.maf.gz','.BROCA.maf.gz')
	out_vcf = filename.replace('raw/MAFs/pWES','processed/MAFs/pWES_BROCA_germ').replace('.maf.gz','.BROCA.vcf')
